...
def calculate_comfort_level(temp, humidity, co2):
    """
    ...
    if temp is None or humidity is None or co2 is None:
        return None
    return round(50 + (temp - 72) + (humidity - 42) + (co2 - 500) / 25 if co2 > 500 else 0)
...
@app.route('/building/<bid>', methods=['GET'])
...
@app.route('/building/<bid>/floor', methods=['GET'])
def get_floors(bid):
    include_rooms = request.args.get('includeRooms', 'false').lower() == 'true'
...
@app.route('/building/<bid>/floor/<fid>', methods=['GET'])
...
@app.route('/building/<bid>/floor/<fid>/room/<rid>/sensor/<sid>', methods=['POST'])
def update_sensor_data(bid, fid, rid, sid):
    ...
    comfort_level = calculate_comfort_level(room.temperature, room.humidity, room.co2)
